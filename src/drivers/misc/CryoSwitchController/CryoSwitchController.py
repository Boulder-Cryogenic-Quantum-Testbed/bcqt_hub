...
class Cryoswitch:
    ...
    def __init__(self, debug=False, COM_port='', IP="192.168.0.117", SN=None, override_abspath=False):
        self.debug = debug
        self.port = COM_port
        self.IP = IP
        self.verbose = True
    ...
    """ do not use smart_connect, thanks :)  -jorge """
...
if __name__ == "__main__":
    ...
    # switch.disconnect(port='A', contact=3) ## Disconnects contact X of switch A from the common terminal
    # switch.disconnect(port='B', contact=3) 
    
    # switch.connect(port='A', contact=4) ## Connect contact X of switch A to the common terminal
    # switch.connect(port='B', contact=4)
