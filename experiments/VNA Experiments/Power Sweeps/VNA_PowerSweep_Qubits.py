...
save_dir = Path(r"./data/cooldown59/Line5_ManuelsQubits")
...
for idx, freq in enumerate(all_freqs):  # loop over all resonators
    ...
    print("\n\n\n" + "*"*30)
    print(f"Now measuring {resonator_name}")
    print("*"*30 + "\n\n\n")
    power_freqs = {}
    for power in all_powers:  # for each resonator, loop over all powers
        ...
        Res_PowSweep_Analysis = DataAnalysis(s2p_df, dstr)

        try:
            params, conf_intervals, err, init1, fig = Res_PowSweep_Analysis.fit_single_res(
                data_df=s2p_df, save_dcm_plot=False, plot_title=resonator_name, save_path=res_dir
            )
                
            #### some code for qubits to track moving resonator ####
            fit_freq_GHz = params["w1"].value/1e9
            error_GHz = (params["w1"].max - params["w1"].min)/1e9
            freq_perc_err = ( error_GHz / fit_freq_GHz )
                
            if freq_perc_err < 5:
                old_freq = freq
                freq = fit_freq_GHz*1e9
                display(f"shifting frequency from {old_freq:1.3f} to {freq:1.3f}  (delta_f = {(old_freq-fit_freq_GHz*1e9)/1e6:1.3f} MHz)")
                Measurement_Configs["f_center"] = freq
                
            power_freqs[power] = freq
            Measurement_Configs["f_span"] *= 1.1
            
            display(f"freq = {freq/1e9:1.6f} GHz, {fit_freq_GHz = :1.6f} GHz, Error = {error_GHz*1e3:1.3f} MHz or {freq_perc_err*100:1.3f} %")
            
            ########################################################
            
        except Exception as e:
            print(f"Failed to fit {resonator_name} - Error:     \n{e}")
            pass
        ...
        meas_name = rf"{freq_str}GHz_{power:1.1f}dBm_{num_avgs}avgs".replace(".","p")

        filename, filepath = qh.archive_data(PNA_X, s2p_df, meas_name=meas_name, save_dir=save_dir, expt_category=resonator_name)

    all_f_res_dicts.append(power_freqs)
